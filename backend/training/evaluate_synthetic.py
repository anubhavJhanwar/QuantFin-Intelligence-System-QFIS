"""
QFIS Synthetic Benchmark Evaluation
Evaluates Base Phi-2 vs Fine-tuned (QLoRA) on the synthetic financial QA dataset.
Keeps FinQA evaluation completely untouched.
"""
...
logger.add(LOG_DIR / "evaluate_synthetic.log", rotation="5 MB")
...
def run_subprocess_model(model_key: str, out_path: str):
    script = Path(__file__).parent / "run_one_model_synthetic.py"
...
def print_table(results):
    ...
    w = 14 + 20*len(models)
    print("\n" + "="*w)
    print("  QFIS SYNTHETIC BENCHMARK RESULTS")
    print("="*w)
...
def plot_comparison(results):
    ...
    ax.set_title("QFIS Synthetic: Base vs Fine-tuned", color="white", fontweight="bold")
    ...
    ax2.set_title("Fine-tuned vs Base: Improvement (Synthetic)", color="white", fontweight="bold")
    ...
    plt.tight_layout()
    out = RESULTS_DIR / "synthetic_comparison.png"
...
def run_evaluation():
    logger.info("="*60)
    logger.info("QFIS Synthetic Benchmark — BLEU / ROUGE / EM / F1")
    logger.info("="*60)

    if not (DATA_DIR / "synthetic_test.json").exists():
        logger.error("synthetic_test.json not found. Run generate_synthetic.py first.")
        sys.exit(1)
    ...
    paths = {
        "Base Phi-2":         str(tmp / "qfis_syn_base_preds.json"),
        "Fine-tuned (QLoRA)": str(tmp / "qfis_syn_ft_preds.json"),
    }
    keys = {"Base Phi-2":"base", "Fine-tuned (QLoRA)":"ft"}
    ...
    final = {
        ...
        "eval_samples": 40,
        "dataset": "synthetic",
        "note": "Controlled single-step financial QA. FinQA results in evaluation_results.json.",
    }
    out = RESULTS_DIR / "synthetic_evaluation_results.json"
    with open(out, "w", encoding="utf-8") as f:
        json.dump(final, f, indent=2, ensure_ascii=False)
    logger.info(f"Results saved: {out}")
    logger.info("FinQA results untouched at evaluation_results.json")
